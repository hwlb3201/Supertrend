'''
connecting to dydx
Build a supertrend strategy for AVAX --- Buy 
'''
      ...
      try:
           ...
           if side=='LONG':
                ...
                if ret < -0.05:
                      ...
                      send_message(f'The stop loss is triggered -kill the {side} position at {Ask_price}')
                else:
                      if candle['Close'][-1]>candle['MA_200'][-1]:
                            if candle['up_trend'][-1]==False:
                                  client.private.cancel_all_orders()
                                  place_sell_order(client,position_id,market,Ask_price,expiration)
                                  send_message(f'Exist the {side} position at {Ask_price}')
                            else:
                                  print('Keep the position')
                      elif candle['Close'][-1]<candle['MA_200'][-1]:
                            print('Kill the position')
                            client.private.cancel_all_orders()
                            place_sell_order(client,position_id,market,Ask_price,expiration)
                            send_message(f'Exist the {side} position at {Ask_price}')
                      else:
           ...
           if side=='SHORT':
                ...
                if ret<-0.1:
                      ...
                      send_message(f'The stop loss is triggered -kill the {side} position at {Bid_price}')
                else:
                      if candle['Close'][-1]<candle['MA_200'][-1]:
                            if candle['up_trend'][-1]==True:
                                  client.private.cancel_all_orders()
                                  place_buy_order(client,position_id,market,Bid_price,expiration)
                                  send_message(f'Kill the {side} position at {Bid_price}')
                            else:
                                  print('Keep the position')
                      if candle['Close'][-1]>candle['MA_200'][-1]:
                            client.private.cancel_all_orders()
      ...
      try: 
            ...
            if candle['Close'][-1]>candle['MA_200'][-1]:
                  if candle['up_trend'][-1]==True:
                        ...
                        Bid_price=bid_ask(client,market)[0]
                        place_buy_order(client,position_id,market,Bid_price,expiration)
                        send_message(f'Placing Buy order at {Bid_price}')
                  else:
                      client.private.cancel_all_orders()
            ...
            if candle['Close'][-1]<candle['MA_200'][-1]:
                  if candle['up_trend'][-1]==False:
                        ...
                        Ask_price=bid_ask(client,market)[1]
                        place_sell_order(client,position_id,market,Ask_price,expiration)
                        send_message(f'Placing Sell order at {Bid_price}')
                  else:
                      print('There is no signal for Short the coins')
            else:
                  print('Wait for 15 mins to see')
